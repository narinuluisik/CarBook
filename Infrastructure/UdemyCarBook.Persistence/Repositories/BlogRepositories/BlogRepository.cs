...
namespace UdemyCarBook.Persistence.Repositories.BlogRepositories
{
    public class BlogRepository : IBlogRepository
    {
        ...
        {
            var blogs = _carBookContext.Blogs.Include(x => x.Author).Include(x => x.Category).ToList();
            return blogs;
        }

        public  List<Blog> GetBlogByAuthorId(int id)
        {
          var value = _carBookContext.Blogs.Include(x => x.Author).Where(y => y.BlogId == id).ToList();
            return value;
        }
