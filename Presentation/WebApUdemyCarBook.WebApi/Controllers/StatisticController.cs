...
namespace WebApUdemyCarBook.WebApi.Controllers
{
    ...
    public class StatisticController : ControllerBase
    {
        ...
        [HttpGet("GetAvgPriceForDaily")]
        public async Task<IActionResult> GetAvgPriceForDaily()
        {
            var values = await _mediator.Send(new GetAvgRentPriceForDailyQuery());
            return Ok(values);
        }
        [HttpGet("GetAvgPriceForMonthly")]
        public async Task<IActionResult> GetAvgPriceForMonthly()
        {
            var values = await _mediator.Send(new GetAvgRentPriceForMonthlyQuery());
            return Ok(values);
        }
        [HttpGet("GetBlogTitleByMaxBlogComent")]
        public async Task<IActionResult> GetBlogTitleByMaxBlogComent()
        {
            var values = await _mediator.Send(new GetBlogTitleByMaxBlogComentQuery());
            return Ok(values);
        }
        [HttpGet("GetAvgPriceForWeekly")]
        public async Task<IActionResult> GetAvgPriceForWeekly()
        {
            var values = await _mediator.Send(new GetAvgPriceForWeeklyQuery());
            return Ok(values);
        }
        ...
        [HttpGet("GetBrandNameWithMaxCar")]
        public async Task<IActionResult> GetBrandNameWithMaxCar()
        {
            var values = await _mediator.Send(new GetBrandNameWithMaxCarQuery());
            return Ok(values);
        }
