    ...
    public class AdminFooterAddressController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<List<ResultFooterAddressDto>>(jsonData, new JsonSerializerSettings
                {
        ...
        public async Task<IActionResult> CreateFooterAddress(CreateFooterAddressDto createFooterAddressDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7087/api/FooterAdresses", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7087/api/FooterAdresses/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminFooterAddress", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateFooterAddress(int id)
        {
            ...
            var responseMessage = await client.GetAsync($"https://localhost:7087/api/FooterAdresses/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateFooterAddress(UpdateFooterAddressDto updateFooterAddressDto)
        {
            ...
            var responseMessage = await client.PutAsync($"https://localhost:7087/api/FooterAdresses/{updateFooterAddressDto.FooterAdressID}", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminFooterAddress", new { area = "Admin" });
            }
