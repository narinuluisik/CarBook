    ...
    public class AdminCommentController : Controller
    {
        ...
        public async Task<IActionResult> Index(int id )
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<List<ResultCommentDto>>(jsonData, new JsonSerializerSettings
                {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7087/api/Comments/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
